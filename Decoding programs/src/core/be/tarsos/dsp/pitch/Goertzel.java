...
public class Goertzel implements AudioProcessor {
	 ...
	 */
	private static final double POWER_THRESHOLD = 3;// in dB

	/**
	 * A list of frequencies to detect.
	 */
	public double[] frequenciesToDetect;
	/**
	...
	public static interface FrequenciesDetectedHandler {
		...
		void handleDetectedFrequencies(final double[] frequencies,
				...
				final double allPowers[], final float[] audioData);
	}
	...
	public boolean process(AudioEvent audioEvent) {
		...
		if (numberOfDetectedFrequencies > 0) {
			...
			double [][] results = new double[numberOfDetectedFrequencies][2];
			float[] audioData = audioFloatBuffer.clone();
			int index = 0;
			for (int j = 0; j < frequenciesToDetect.length; j++) {
				if (calculatedPowers[j] > POWER_THRESHOLD) {
					frequencies[index] = frequenciesToDetect[j];
					powers[index] = calculatedPowers[j];
					results[index][0] = frequenciesToDetect[j];
					results[index][1] = calculatedPowers[j];
					index++;
				}
			}

			handler.handleDetectedFrequencies(frequencies, powers,
					frequenciesToDetect.clone(), calculatedPowers.clone(), audioData);
		}
	...
	public void setFrequenciesToDetect(double[] _frequencies){
		frequenciesToDetect = _frequencies;
	}
}
