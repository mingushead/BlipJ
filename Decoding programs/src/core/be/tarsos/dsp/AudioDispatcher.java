...
public class AudioDispatcher implements Runnable {
	...
	public void run() {
		...
		//	System.out.println("in the middle " + (System.currentTimeMillis() % 1000));
			if(!stopped){			
	...
	private int readNextAudioBlock() throws IOException {
		//System.out.println(System.currentTimeMillis() % 1000 + " here we go");
		...
		//else if(bytesToRead
			...
			offsetInBytes = byteOverlap * iteration;
			offsetInSamples = floatOverlap * iteration;
		}
		...
		while(!stopped && !endOfStream && totalBytesRead<bytesToRead){
			try{
				...
				//System.out.println("Before read:  \t" +(System.currentTimeMillis() % 1000));
			bytesRead =	 audioInputStream.read(audioByteBuffer, (offsetInBytes*iteration) + totalBytesRead , (bytesToRead - totalBytesRead));
				//System.out.println("After read:  \t" +(System.currentTimeMillis() % 1000));
				//System.out.println("audio byte buffer length after read:  \t" + audioByteBuffer.length);
				//System.out.println("offset: \t" + offsetInBytes);
				//System.out.println(System.currentTimeMillis() % 1000 + " halfway there");
			}catch(IndexOutOfBoundsException e){
				// The pipe decoder generates an out of bounds if end
				// of stream is reached. Ugly hack...
				bytesRead = -1;
				//System.out.println("Total bytes read: \t" + -1);
			}
			if(bytesRead == -1){
		...
		//System.out.println(System.currentTimeMillis() % 1000 + " after while");
		if(endOfStream){
		//	System.out.println(System.currentTimeMillis() % 1000 + " ended");
			...
			//System.out.println(System.currentTimeMillis() % 1000 + " all bytes read");
