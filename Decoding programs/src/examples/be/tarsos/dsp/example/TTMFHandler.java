...
public class TTMFHandler {
    ...
    TTMFHandler(double[]input, int[]dims){
        ...
        frequencies = input;
        dimensions = dims;
        output = new int[dims[0]][dims[1]][dims[2]];
        ...
        //      p    q    r    s    t    u    v    w    x    y    z    ~    ,   ?   !   .   *   $   ^   SP  #
                112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 126, 44, 63, 33, 46, 42, 36, 94, 32, 35};

        int placeholder = 0;
        for(int i = 0; i < dims[0]; i++){
            for(int j = 0; j < dims[1]; j++){
                for(int k = 0; k < dims[2]; k++){
                    output[i][j][k] = charIndex[placeholder];
                    placeholder++;
                }
            }
        }
       // output[dims[0]-1][dims[1]-1][dims[2]-1] = 46;
        //output[1][2][1] = 32;

    }

    public char parseTTMF(int[] detectedFreqs){
        char result;
        ...
        result = (char) output[location[0]][location[1]][location[2]];
        return result;
    }

    public TTMFResult parseTTMF(double[] detectedFreqs){
        char resultLoc;
        for(int i = 0; i < frequencies.length; i++){
            if(detectedFreqs[0] == frequencies[i]){
                location[0] = i;
       ...
       // System.out.println("Loc: " + location[0] + "\t"+ location[1] + "\t"+ location[2] + "\t");
        ...
        if(location[0] >= 0 && location[1] >= 0 && location[2] >= 0) {
            try {
                resultLoc = (char) output[location[0]][location[1]][location[2]];
            }
            catch(Exception e){
                System.out.println(location[0] + "\t" + location[1] + "\t" + location[2] + "\t");
            }
        }
        TTMFResult result = new TTMFResult(resultLoc, ((location[0]+1)*3) + (location[1]*4) + (location[2]));
        return result;
    }
}
class  TTMFResult {
    ...
    public TTMFResult (char ch, int co){
        this.charResult = ch;
        this.code = co;
    }
}
